...
interface ChatStore {
  ...
  newlyCreatedMessageIds: Set<string>; // Track new messages for typewriter effect
  ...
  markMessageAsNew: (messageId: string) => void;
  clearNewMessageMark: (messageId: string) => void;
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  newlyCreatedMessageIds: new Set(),
  ...
  addMessageToChat: (chatId, message) => {
    const { chatMessages, newlyCreatedMessageIds } = get();
    set({
      chatMessages: {
        ...chatMessages,
        [chatId]: [...(chatMessages[chatId] || []), message],
      },
      // Mark the message as new for typewriter effect
      newlyCreatedMessageIds: new Set([...newlyCreatedMessageIds, message.id]),
    });
  },
  ...
  markMessageAsNew: (messageId: string) => {
    const { newlyCreatedMessageIds } = get();
    set({
      newlyCreatedMessageIds: new Set([...newlyCreatedMessageIds, messageId]),
    });
  },

  clearNewMessageMark: (messageId: string) => {
    const { newlyCreatedMessageIds } = get();
    const updatedSet = new Set(newlyCreatedMessageIds);
    updatedSet.delete(messageId);
    set({ newlyCreatedMessageIds: updatedSet });
  },
  ...
  resetToInitial: () => {
    ...
    set({
      ...
      pendingUserMessage: null,
      newlyCreatedChatIds,
      newlyCreatedMessageIds: new Set(), // Clear new message marks when resetting
    });
  },
}));
